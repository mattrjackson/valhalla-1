#ifndef VALHALLA_SIF_HAZARDCOST_H_
#define VALHALLA_SIF_HAZARDCOST_H_
...
public:
    std::function<double(const baldr::DirectedEdge* , const baldr::GraphTile* , const uint32_t, Cost& , const bool)> ComputeEdgeCostF = 0;    
    std::function<double(const baldr::GraphId id, const uint32_t start, Cost&, const bool)> ComputeNodeCostF = 0;
    cost_ptr_t GetSubCosting()
    {
        return cost_;
    }
  /**
  ...
  virtual Cost EdgeCost(const baldr::DirectedEdge* edge,
     ...
     throw std::runtime_error("HazardCost::EdgeCost does not support transit edges");
  }
    /**
                        ...
                        const uint32_t seconds) const
  {
    ...
    if(ComputeEdgeCostF != 0 )
       cost.cost = ComputeEdgeCostF(edge, tile, seconds, cost, false);
    return cost;
  }
  /**
                        ...
                        const uint32_t seconds) const
  {
    ...
    if(ComputeEdgeCostF != 0 )
      cost.cost = ComputeEdgeCostF(edge, tile, seconds, cost, true);
    return cost;
  }                        
                              ...
                              const EdgeLabel& pred) const
  {
    ...
    if(ComputeNodeCostF != 0 )
      cost.cost = ComputeNodeCostF(pred.endnode(), pred.cost().secs, cost, true );
    return cost;
  }                              
   std::shared_ptr<CostConstraints> GetConstraints() {
    return cost_ -> GetConstraints();
  }
                                     ...
                                     const baldr::DirectedEdge* edge) const
  {
    ...
    if(ComputeNodeCostF != 0 )
      cost.cost = (float)ComputeNodeCostF(pred->endnode(), 0, cost, false );
    return cost;
  }
  ...
  {
    return cost_ -> AStarCostFactor() * heuristic_multiplier; 
  }

  inline bool ConstraintsSatisfied(const Cost& cost) const {
    return cost_ -> GetConstraints() -> ConstraintsSatisfied(cost);
  }
  /**
...
#endif // VALHALLA_SIF_HAZARDCOST_H_
